...
#include <sys/socket.h>
 struct stun_binding *  stun_bind_request(char *src_ip) {
  ...
  struct sockaddr_in *sockaddr = get_network_socket(stun_server_ip, 19302);

  int bytes = sendto(sock_desc, &stun_request, sizeof(stun_request), 0,
                     (struct sockaddr *)sockaddr, sizeof(struct sockaddr_in));
  printf("stun packet sent server :%s with local IP: %s \n",
         stun_server_ip, src_ip);
  char *udp_packet = malloc(1000);
  bytes = recvfrom(sock_desc, udp_packet, 1000, 0, (struct sockaddr *)sockaddr,
                   &socklen);
  ...
  uint16_t mapped_port =  (ntohs(stun_respose_payload->x_port ) ^ ((uint16_t) (ntohs(stun_respose->magic_cookie)))); //port and ip is sotred xored with the cookie 
  uint32_t mapped_ip =  (stun_respose_payload->x_ip  ^ ((stun_respose->magic_cookie)));
